...
const validarRegistro = async (req, res, next) => {
    ...
    if (typeof registro !== 'object') return res.status(400).send('Error en el formato de los datos enviados')
    ...
    if (!chequeoPropiedades) return res.status(400).send('Los datos enviados deben contener únicamente nombre y edad');

    // Condiciones sobre el nombre y la edad
    const registroNombre = registro.nombre
    const registroEdad = registro.edad

    if (registroNombre.length < 3 || registroNombre.length > 50) return res.status(400).send('El nombre debe tener entre 3 y 50 caracteres');

    if (registroEdad < 1 || registroEdad > 130) return res.status(400).send('La edad debe ser un número entero positivo menor que 130');
    ...
    if (duplicado) return res.status(400).send('El nombre ya se encuentra registrado');
